...
public class IntSetTest {
    ...
    public void union() throws Exception {
        ...
        for (int j = 0; j < union.getData().length; j++) {
            assertEquals(union.getData()[j], small.union(big).getData()[j]);
        }
    }
    ...
    public void intersect() throws Exception {
        ...
        for (int j = 0; j < intersect.getData().length; j++) {
            assertEquals(intersect.getData()[j], small.intersect(big).getData()[j]);
            assertEquals(intersect.getData()[j], big.intersect(small).getData()[j]);
        }
    }
    ...
    public void difference() throws Exception {
        ...
        for (int j = 0; j < diff.getData().length; j++) {
            assertEquals(diff.getData()[j], big.difference(small).getData()[j]);
        }
    }
